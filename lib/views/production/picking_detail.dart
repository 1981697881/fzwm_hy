...
//下推
      ...
      var entryId = [];
      var hobbyIndex = 0;
      this.orderDate.forEach((element) {
        entryId.add(element[5]);
        hobbyIndex++;
      });
    ...
    if (orderDate.length > 0) {
      orderDate.forEach((value) {
        ...
        arr.add({
          "title": "生产日期",
          "name": "FProduceDate",
          "isHide": value[18] != true,
          "value": {
            "label": value[16] == null ? '' : value[16].substring(0, 10),
            "value": value[16] == null ? '' : value[16].substring(0, 10)
          }
        });
        arr.add({
          "title": "有效期至",
          ...
          "isHide": value[18] != true,
          "value": {
            "label": value[17] == null ? '' : value[17].substring(0, 10),
            "value": value[17] == null ? '' : value[17].substring(0, 10)
          }
        });
        hobby.add(arr);
      });
  ...
  getMaterialList(barcodeData, code, fsn, fProduceDate, fExpiryDate, fBatchNo) async {
    Map<String, dynamic> userMap = Map();
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    var menuData = sharedPreferences.getString('MenuPermissions');
    var deptData = jsonDecode(menuData)[0];
      ...
      if (number == 0 && this.fBillNo == "") {
        materialDate.forEach((value) {
          ...
          arr.add({
            ...
            "isHide": value[6] != true,
            "value": {
              "label": value[6] ? (scanCode.length > 1 ? fBatchNo : '') : '',
              "value": value[6] ? (scanCode.length > 1 ? fBatchNo : '') : ''
            }
          });
          arr.add({
            "title": "需生产数量",
      ...
      onConfirm: (p) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (data[i] == p) {
            ...
            hobby['value']['dimension'] = stockListObj[i][4];
            break;
          }
        }
      },
