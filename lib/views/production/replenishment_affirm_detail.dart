...
class ReplenishmentAffirmDetail extends StatefulWidget {
  var FBillNo;

  ReplenishmentAffirmDetail({Key? key, @required this.FBillNo})
      : super(key: key);

  @override
  _SalesReturnAffirmDetailState createState() =>
      _SalesReturnAffirmDetailState(FBillNo);
}

class _SalesReturnAffirmDetailState extends State<ReplenishmentAffirmDetail> {
  ...
  saveOrder() async {
    if (this.hobby.length > 0) {
      ...
      if (orderDate[0][orderDate[0].length - 1] == "B") {
        ...
        //审核
        HandlerOrder.orderHandler(context, submitMap, 3, "PRD_FeedMtrl",
              ...
              HandlerOrder.orderHandler(context, submitMap, 3, "PRD_FeedMtrl",
                ...
                if (auditResult) {
                  ...
                  HandlerOrder.orderHandler(context, submitMap, 3,
                          "PRD_FeedMtrl", SubmitEntity.unAudit(submitMap))
                      .then((unAuditResult) {
                    if (unAuditResult) {
                      this.isSubmit = false;
                    } else {
