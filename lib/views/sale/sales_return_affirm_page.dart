...
import 'package:fzwm_hy/views/purchase/purchase_return_detail.dart';
import 'package:fzwm_hy/views/sale/sales_return_affirm_detail.dart';
import 'package:qrscan/qrscan.dart' as scanner;
import 'package:shared_preferences/shared_preferences.dart';

final String _fontFamily = Platform.isWindows ? "Roboto" : "";

class SalesReturnAffirmPage extends StatefulWidget {
  SalesReturnAffirmPage({Key ?key}) : super(key: key);

  @override
  _ReturnGoodsPageState createState() => _ReturnGoodsPageState();
}

class _ReturnGoodsPageState extends State<SalesReturnAffirmPage> {
  //搜索字段
  String keyWord = '';
  String startDate = '';
  String endDate = '';
  ...
  getOrderList() async {
    ...
    if(this.isScan){
      userMap['FilterString'] = "FDocumentStatus in ('A','B','D')";
      if(this.keyWord != ''){
        userMap['FilterString'] = "FBillNo like '%"+keyWord+"%' and FDocumentStatus in ('A','B','D')";
      }
    }else{
      if(this.keyWord != ''){
        userMap['FilterString'] = "FBillNo like '%"+keyWord+"%' and FDocumentStatus in ('A','B','D')";
      }else{
        userMap['FilterString'] = "FBillNo like '%"+keyWord+"%' and FDocumentStatus in ('A','B','D') and FDate>= '$startDate' and FDate <= '$endDate'";
      }
    }
    this.isScan = false;
                ...
                child: ListTile(
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) {
                          return SalesReturnAffirmDetail(FBillNo:this.hobby[i][0]['value']
                            // 路由参数
                          );
                        },
                      ),
